...
int occupy_quantity;
cell waiting_processes[MAXQUEUE];
int wait_quantity;
...
int readData()
{
  ...
  strcpy(fname, "resource_allocation_queue.txt");
  // cin>>fname;
  ...
  strcpy(fname, "resource_wait_queue.txt");
  // cin>>fname;
  if ((fp = fopen(fname, "r")) == NULL)
  {
    cout << "错误,文件打不开,请检查文件名:)" << endl;
  ...
  else
  {
    while (!feof(fp))
    {
      fscanf(fp, "%d %d", &waiting_processes[wait_quantity].process, &waiting_processes[wait_quantity].resource);
      wait_quantity++;
    }
  }
  ...
  {
    cout << "  " << waiting_processes[i].resource << "     " << waiting_processes[i].process << endl;
  }
  return 1;
}
...
void check()
{
  ...
  for (i = 0; i < wait_quantity; i++)
  {
    ...
    {
      if (waiting_processes[i].resource == occupy[j].resource)
      {
        table[waiting_processes[i].process][occupy[j].process] = 1;
        table1[waiting_processes[i].process][occupy[j].process] = 1;
      }
    }
  }
